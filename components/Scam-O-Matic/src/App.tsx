...
const calculateWins = (grid: SymbolType[][], betPerLine: number, activeLines: number) => {
  ...
  for (let i = 0; i < activeLines; i++) {
    ...
    // Check for L-O-S-E-R combination
    ...
    for (let col = 0; col < 5; col++) {
      ...
      if (col === 0) {
        ...
        if (symbol === matchSymbol || symbol === 'HOUSE') {
          matchCount++;
          positions.push({ col, row });
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (spinningCols.some(c => c)) {
      interval = setInterval(() => {
        setDisplayGrid(prev => prev.map((col, i) => 
          spinningCols[i] ? [getRandomSymbol(), getRandomSymbol(), getRandomSymbol()] : col
        ));
      }, 80);
    }
  ...
  useEffect(() => {
    if (freeSpins > 0 && !isSpinning && winAmount === 0 && !gambleModalOpen) {
      const timer = setTimeout(() => {
        spin();
      }, 1000);
      return () => clearTimeout(timer);
    }
  }, [freeSpins, isSpinning, winAmount, gambleModalOpen]);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-zinc-900 text-white font-sans p-4">
      <div className="bg-zinc-800 p-6 rounded-3xl shadow-2xl border-4 border-zinc-700 max-w-5xl w-full">
        <div className="text-center mb-6 relative">
          <h1 className="text-4xl md:text-6xl font-black text-red-500 tracking-widest uppercase drop-shadow-[0_0_10px_rgba(239,68,68,0.8)]">
            SCAM-O-MATIC 3000
          </h1>
          ...
          {freeSpins > 0 && (
            <div className="text-2xl text-purple-400 font-bold mt-2 animate-pulse">
                ...
                {col.map((symbol, rowIndex) => {
                  ...
                  return (
                    <div 
                      key={rowIndex} 
                      className={`
                        h-24 md:h-32 flex flex-col items-center justify-center rounded-md
                    ...
                    >
                      {['L', 'O', 'S', 'E', 'R', '!'].includes(symbol) ? (
                        <span className="text-6xl md:text-8xl font-black drop-shadow-[0_0_10px_currentColor]">{SYMBOLS[symbol].label}</span>
                      ) : (
                        <>
                          <span className="text-4xl md:text-6xl mb-1">{SYMBOLS[symbol].emoji}</span>
          ...
          <div className="flex gap-4">
            <div className="flex flex-col items-center">
              ...
              <div className="flex items-center gap-2 bg-black rounded-lg p-1">
                <button onClick={() => setLines(l => Math.max(1, l - 1))} disabled={isSpinning} className="p-2 bg-zinc-800 rounded hover:bg-zinc-700 disabled:opacity-50"><Minus size={16} /></button>
                <span className="w-8 text-center font-bold">{lines}</span>
                <button onClick={() => setLines(l => Math.min(9, l + 1))} disabled={isSpinning} className="p-2 bg-zinc-800 rounded hover:bg-zinc-700 disabled:opacity-50"><Plus size={16} /></button>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="flex items-center gap-2 bg-black rounded-lg p-1">
                <button onClick={() => setBetPerLine(b => Math.max(1, b - 1))} disabled={isSpinning} className="p-2 bg-zinc-800 rounded hover:bg-zinc-700 disabled:opacity-50"><Minus size={16} /></button>
                <span className="w-8 text-center font-bold">{betPerLine}</span>
                <button onClick={() => setBetPerLine(b => Math.min(10, b + 1))} disabled={isSpinning} className="p-2 bg-zinc-800 rounded hover:bg-zinc-700 disabled:opacity-50"><Plus size={16} /></button>
              </div>
            </div>

            <div className="flex flex-col items-center justify-center">
              <button 
                onClick={() => { setLines(9); setBetPerLine(10); }}
                disabled={isSpinning}
